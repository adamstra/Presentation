...
def load_data(place_name):
    ...
    chemin_shp = f"/Users/AdamsTra/Desktop/HACKHATON/{place_name}/Batis.shp"
...
def main():
    # Choix de la ville
    selected_city = st.selectbox('Choisir la ville', ['Keur Massar', 'Diourbel'])

    # Chargement des données
    gdf = load_data(selected_city)
    ...
    graph = ox.graph_from_address(selected_city, network_type='walk')

    # Affichage du graphique dans Streamlit
    st.title("Graphique du réseau OSM")
    st.write("Réseau OSM pour la zone :", selected_city)
    ...
    buildings = ox.geometries.geometries_from_address(selected_city, tags={'building': True}, dist=dist_range)
    ...
    st.write("Empreintes des bâtiments pour la zone :", selected_city)
    ...
    # Récupération des empreintes des bâtiments
    buildings = ox.geometries.geometries_from_address(selected_city, tags={'building': True}, dist=1500)
